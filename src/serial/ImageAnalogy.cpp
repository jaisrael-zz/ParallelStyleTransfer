...
double ImageAnalogy::l2_norm(const vector<uchar>& F1, const vector<uchar>& F2)
{
  double sum = 0;
...
void ImageAnalogy::create_feature_vector(vector<uchar>& F, 
                                         ...
                                         const Displacement& disp)
{
  ...
  int down_displace = disp.up_fine;
  int left_displace = disp.left_fine;
  int right_displace = disp.right_fine;

  F.clear();
  int prime_flag = 0;
  for (int r = (row - up_displace); r <= (row + down_displace); r++)
  {
    ...
    for (int c = col-left_displace; c <= right_displace+col; c++)
    {
      ...
      F.push_back(*m_row_ptr);
      if (!prime_flag) F.push_back(*mp_row_ptr);
      mp_row_ptr++;
      m_row_ptr++;
    }
  }
                               ...
                               const int l, const Point q)
{
  ...
  Point p_a  = best_approximate_match(IP, l, q, FQ, disp);
 
  /*Point p_c = best_coherence_match(GPA, GPA_p, GPB, GPB_p, PS, q, FQ,
                                     up_displace, down_displace, left_displace,
                                     right_displace);*/
  return p_a;
}
